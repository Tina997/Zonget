...
public class AdministratorUsersAddPetPresenter
  ...
  public AdministratorUsersAddPetPresenter(AdministratorUsersAddPetState state) {
    AdministratorUsersAddPetViewModel viewModel = state;
  }

  @Override
  public void injectView(WeakReference<AdministratorUsersAddPetContract.View> view) {
    WeakReference<AdministratorUsersAddPetContract.View> view1 = view;
  }
  ...
  public void insertNewPet(String name, String species, String breed, String chipNum, String birthday) {
    ...
    model.insertNewPet(userID, name, species, breed, chipNum, birthday,
            new RepositoryContract.Accounts.InsertNewUserPetCallback() {
              @Override
  ...
  @Override
  public boolean checkAllDataIntroduced(Editable name, Editable species, Editable breed,
                                        Editable chip, Editable birthday) {
    if (name.length() == 0 || species.length() == 0 || breed.length() == 0 || chip.length() == 0
            || birthday.length() == 0) {
      return false;
    }
    return true;
